...
export class ProjectLoader extends LitElement {
  ...
  loadProject() {
    ...
    fetch(`${this.folder}/manifest.json`).then(response => {
      ...
      return this.getFiles(manifest).then(filesArray => {
        ...
        this.project = Object.assign({}, manifest, {
          files: filesObject
        });
  ...
  getFiles(manifest) {
    return Promise.all(manifest.files.map(filename => {
      return fetch(`${this.folder}/${filename}`).then(result => {
        return result.text().then(r => {
          return {
            [filename]: this.uglyHack(r)
          };
        }).catch(error => {
          console.log(error);
        });
